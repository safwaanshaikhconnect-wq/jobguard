...
export default function LiveTerminal({ logs }: LiveTerminalProps) {
  ...
  return (
    <div className="h-full flex flex-col font-mono bg-[#050505] border border-[#1a1a1a] rounded-sm overflow-hidden shadow-2xl animate-in zoom-in-95 duration-500">
      {/* Terminal Header */}
      <div className="bg-[#0a0a0a] px-4 py-3 border-b border-[#1a1a1a] flex items-center justify-between select-none">
        <div className="flex items-center gap-2">
          <TerminalIcon className="w-4 h-4 text-[#ef4444]" />
          <span className="text-[10px] uppercase tracking-[0.4em] font-bold text-[#f5f5f5]">RAW_INTELLIGENCE_STREAM_v.2.0</span>
        </div>
        <div className="flex items-center gap-6 text-[9px] uppercase tracking-widest text-[#2a2a2a] font-bold">
            <span className="flex items-center gap-2 text-[#22c55e]"><div className="w-1.5 h-1.5 rounded-full bg-[#22c55e] animate-pulse" /> SENSOR_ARMED</span>
            <span>NODE_ID: JG-4491</span>
            <span className="text-[#f5f5f5] bg-[#ef4444] px-1.5 py-0.5 rounded-[2px]">SECURE</span>
        </div>
      </div>
      ...
      <div 
        ref={scrollRef}
        className="flex-1 overflow-y-auto p-6 space-y-1.5 bg-[#050505] scrollbar-thin scrollbar-thumb-[#1a1a1a]"
      >
        {logs.map((log) => (
          <div key={log.id} className="flex gap-4 text-[11px] leading-relaxed group hover:bg-[#111111]/50 transition-colors py-0.5 relative">
      ...
      {/* Footer / Stats */}
      <div className="bg-[#0a0a0a] px-5 py-2 border-t border-[#1a1a1a] flex gap-10 select-none opacity-40">
        <div className="flex items-center gap-2">
          <Database className="w-3 h-3 text-[#737373]" />
          <span className="text-[9px] uppercase tracking-[0.2em] font-bold">Buffer: {logs.length}/100</span>
        </div>
        <div className="flex items-center gap-2">
            <Cpu className="w-3 h-3 text-[#737373]" />
            <span className="text-[9px] uppercase tracking-[0.2em] font-bold font-mono">Kernel_0.4.1_stable</span>
        </div>
        <div className="flex items-center gap-2 ml-auto">
            <Command className="w-3 h-3 text-[#737373]" />
            <span className="text-[9px] uppercase tracking-[0.2em] font-bold">X86_64_INST_READY_V2</span>
        </div>
      </div>
    </div>
  );
